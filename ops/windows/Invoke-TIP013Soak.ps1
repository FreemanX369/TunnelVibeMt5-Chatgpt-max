...
function Get-ExactTunnel{param([string]$Exe,[string]$Profile)$expected=[IO.Path]::GetFullPath($Exe);$pattern='--profile(?:\s+|=)"?'+[regex]::Escape($Profile)+'"?(?:\s|$)';@(Get-CimInstance Win32_Process -Filter "Name='tunnel-client.exe'" -ErrorAction SilentlyContinue|Where-Object{$_.ExecutablePath -and ([IO.Path]::GetFullPath([string]$_.ExecutablePath)-ieq $expected) -and $_.CommandLine -and $_.CommandLine -match $pattern})}
