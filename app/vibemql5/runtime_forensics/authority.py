...
def canonical_payload(record: AuthorityRecord) -> bytes:
    return json.dumps(record.unsigned_dict(), sort_keys=True, separators=(",", ":"), ensure_ascii=False).encode("utf-8")
...
def validate_live_binding(record: AuthorityRecord, *, observed_pid: int, observed_creation_time_100ns: int, observed_image_path: str) -> None:
