...
def compare_pages(a: dict[int, dict], b: dict[int, dict]) -> dict:
    ...
    for address in sorted(set(a) | set(b)):
        ...
        if pa["sha256"].lower() != pb["sha256"].lower() or int(pa["size"]) != int(pb["size"]):
            changed.append({"address": address, "before": pa, "after": pb})
        else:
            unchanged += 1
    return {"summary": {"pages_a": len(a), "pages_b": len(b), "added": len(added), "removed": len(removed), "changed": len(changed), "unchanged": unchanged}, "added": added, "removed": removed, "changed": changed}
...
def main() -> int:
    ...
    a = load_pages(args.a)
    b = load_pages(args.b)
    result = compare_pages(a, b) if args.mode == "delta" else {"summary": {"target_specific": len(target_specific(a, b))}, "pages": target_specific(a, b)}
