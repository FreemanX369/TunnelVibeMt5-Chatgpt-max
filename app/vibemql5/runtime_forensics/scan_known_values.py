        ...
        for representation, needle, normalized in patterns_for(requested):
            ...
            for page in pages:
                ...
                for local in find_all(page_blob, needle, remaining):
                    results.append({"requested": requested, "normalized": normalized, "representation": representation, "address": hex(page["address"] + local), "page_address": hex(page["address"]), "page_offset": local, "sidecar_offset": begin + local, "protect": page.get("protect")})
                    hit_count += 1
    summary = {}
    for hit in results:
